...
color ray_color(const ray& r) {
	auto t = hit_sphere(point3(0, 0, -0.8), 0.5, r);
	if (t > 0.0) {
		vec3 N = unit_vector(r.at(t) - vec3(0, 0, -0.8));
...
int main() {
	...
	auto focal_length = 1.0; // distance of viewport from camera
	...
	for (int j = 0; j < image_height; j++) {
		...
		for (int i = 0; i < image_width; i++) {
			...
			ray r(camera_center, ray_direction); // rays shoot out of the camera toward each pixel
